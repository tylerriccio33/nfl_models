...
data <- nflreadr::load_pbp(2015:2022) %>%
  # Rename IDs
  rename(id_game = game_id,
         id_posteam = posteam,
         id_season = season,
         id_week = week) %>%
  drop_na(starts_with('id_'), success, epa, down) %>%
  filter(
...
# Stability Result
res_lookup <- data %>%
  filter(id_week >= 9 & id_week <= 16) %>%
  select(starts_with('id_'), success, wp) %>%
  # Apply a basic GT filter to the lookup
  filter(wp > .1 | wp < .9) %>%
  # Get Avg Success
  group_by(id_posteam, id_season) %>%
  summarize(sr = mean(success)) %>%
  ungroup()
  
# Classic Result
res_lookup <- data %>%
  filter(id_week <= 8 & id_week <= 16) %>%
  group_by(id_game, id_posteam, id_season) %>%
  slice(1) %>%
  summarize(res = if_else(id_posteam == home_team, result, result * -1)) %>%
  ungroup() %>%
  group_by(id_posteam, id_season) %>%
...
calculate_new_rsq <- function(data, weight, lut = res_lookup) {
  
  ## Specify each condition and iterate the weight conditionally
  
  # Modified Data
  modified_data <- data %>%
    mutate(
      fs = if_else(down == 1, weight * success, success),
      gt = if_else(wp < .1 | wp > .9, weight * success, success),
      fumb = if_else(fumble == 1, success * weight, success),
      rz = if_else(yardline_100 <= 20, success * weight, success),
      pen = if_else(penalty == 1, success * weight, success),
      int = if_else(interception == 1, success * weight, success),
      rush = if_else(rush == 1, success * weight, success)
    )
  
  new_cols <- colnames(modified_data) %>%
    keep( ~ !.x %in% colnames(data))
  
  # Select and rename
  reduced_data <- modified_data %>%
    rename_with(~ glue("{.x}_weight"), .cols = all_of(new_cols)) %>%
    # Reduce
    filter(id_week <= 8) %>%
    group_by(id_season, id_posteam) %>%
    summarize(across(ends_with('_weight'), ~ mean(.x)),
              base_weight = mean(success)) %>%
    ungroup()
  
  # Join
  joined_data <- lut %>%
    left_join(reduced_data, by = c('id_season','id_posteam'))
  
  # Calculate RSQ
  results <- joined_data %>%
    select(res, ends_with('weight')) %>%
    pivot_longer(-res) %>%
    group_by(name) %>%
    summarize(c = cor(res, value)) %>%
    ungroup() %>%
